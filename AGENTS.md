...
3. Give Skyler the local review URL:
   - `http://localhost:4173/` on this Mac
   - the Vite network URL when same-network testing is useful
4. Let Skyler review and interact with the real app against the real Supabase-backed environment from localhost.
5. Only after Skyler approves the behavior should you commit, push, merge, or deploy to the live URL.
...
- Supabase migrations and Edge Function deploys are live changes. If backend work is required, call that out explicitly before applying it so Skyler understands that the data path is no longer local-only.
...
Only Skyler (the owner) merges branches to main. Push to your feature branch only.
