...
const callerName = fields.get("Caller Name")?.text; // "Mack Harper"
```
...
If no profile ID is found in URLs (e.g., a new email type without quotable buttons), fall back to looking up by the "Profile" text field value (e.g., "Adair Forsythe") in the `ignite_config` table:
...
```json
{
  ...
  "sourceDetail": "K9 Resorts - Adair Forsythe (none)",
...
| Single-name caller | "Vance" | `{ firstName: "Vance", lastName: "" }` |
