...
Ignite Visibility Platform
  ↓ sends email to hayden.ellison42@example.com
...
| Adair Forsythe profile ID | `156865` |
| Remy Calloway profile ID | `156866` |
| Account ID | `106348` |
...
- **Example:** `New Phone Call Received | Adair Forsythe`
...
- **Example:** `New Appointment Received | Adair Forsythe`
...
| Profile | Text | No | Location name, e.g., "Adair Forsythe" |
...
**Important: Caller Name is REVERSED.** The format is `{LastName} {FirstName}`, not the typical `{FirstName} {LastName}`. Example: "Mack Harper" means first_name="Harper", last_name="Mack".
...
The profile ID (e.g., `156865` for Adair Forsythe) is **NOT** a visible field in the data table. It only appears embedded in href URLs within the email HTML.
...
```
emFjay5uaXNiZXRAbHBoaWs5LmNvbQ → hayden.ellison42@example.com
```
...
Phone call emails list the caller name as `{Last} {First}` — e.g., "Mack Harper" means Harper Mack. The parser must split and reverse.
