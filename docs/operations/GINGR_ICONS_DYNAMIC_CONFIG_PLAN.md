...
- Runtime compute still falls back to hard-coded icon titles, service strings, reservation type names, room names, workflow IDs, and Adair Forsythe-specific cron payloads.
...
| Rooms/runs | Adair Forsythe room labels/counts, run `is_private_play` / `is_isolation` from name, `type_id = "5"` | `gingr-sync`, `src/shared/theme.js`, `useGingrData.js`, mobile `workflow-reports.ts` |
...
3. Seed suggested mappings
   - Use Adair Forsythe-style text heuristics only as import suggestions.
...
6. Start initial historical sync
   ...
   - Historical pull must include all records since inception, matching the Adair Forsythe approach.
...
- Add tests proving compute does not change Adair Forsythe results before dynamic mappings are enabled.
...
- Replace Adair Forsythe-only cron payloads with a Git-tracked location-aware cron manifest or a scheduler that iterates active configured locations.
...
- Adair Forsythe before/after diff for bathing, private play, playgroups, room cleaning, scheduling capacity, and workflow progress.
...
- Decide whether the first implementation scope should be Adair Forsythe-compatible refactor only, or full onboarding UI in the same branch.
...
8. Adair Forsythe parity tests proving no operational output changed unintentionally.
