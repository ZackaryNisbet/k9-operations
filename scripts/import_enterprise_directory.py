#!/usr/bin/env python3
"""Build deterministic Enterprise Directory import data from Zack's source workbooks."""
...
CORPORATE_DEFAULT = Path("/Users/zacknisbet/Downloads/LPHI Directory.xlsx")
RESORTS_DEFAULT = Path("/Users/zacknisbet/Downloads/Gm Contact List (1).xlsx")

PERSON_ALIASES = {
    "zack.nisbet@lphik9.com": "zack-nisbet",
    "sean.powell@lphik9.com": "sean-powell",
    "lia.moncholi@lphik9.com": "lia-moncholi",
    "alan.leibman@lphik9.com": "alan-leibman",
    "phil.nisbet@lphik9.com": "phil-nisbet",
    "kevin.russell@k9resorts.com": "kevin-russell",
    "kevin.russell@lphik9.com": "kevin-russell",
    "johnny.meeth@lphik9.com": "johnny-meeth",
    "stevyn.dockery@lphik9.com": "stevyn-dockery",
}

DIRECT_TO_SEAN_NOTE = "No separate regional manager listed."
...
def parse_resorts(path: Path, people: dict[str, dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], set[str]]:
    ...
    # Workbook summary table says CA/AZ regional manager is Stevyn, while rows
    # currently point CA/AZ resorts to Johnny. Preserve that contradiction.
    gaps.append({
...
def build_edges(people: dict[str, dict[str, Any]], assignments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    add("alan-leibman", "lia-moncholi", "hierarchy_rule", True, "Lia reports to both Co-CEOs; Alan is primary for table edge ordering.")
    add("phil-nisbet", "lia-moncholi", "hierarchy_rule", False, "Lia reports to both Co-CEOs.")
    add("lia-moncholi", "sean-powell", "hierarchy_rule", True)
    add("sean-powell", "zack-nisbet", "hierarchy_rule", True, "Zack is Director of Resorts and GM for Cherry Hill; represented once.")

    for key in ("kevin-russell", "johnny-meeth", "stevyn-dockery"):
        if key in people:
            add("sean-powell", key, "hierarchy_rule", True)
    ...
    for location_key, gm_key in gm_by_location.items():
        ...
        if gm_key == "zack-nisbet":
            continue
        if regional_key == "sean-powell" or not regional_key:
            add("sean-powell", gm_key, "gm_contact_list", True, DIRECT_TO_SEAN_NOTE)
        else:
...
def build_dataset(corporate: Path, resorts: Path) -> dict[str, Any]:
    ...
    # Zack is a single directory person with Cherry Hill responsibility attached.
    zack = people.get("zack-nisbet")
    if zack:
        zack["title"] = "Director of Resorts / General Manager, Cherry Hill"
