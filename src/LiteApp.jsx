...
function classifyReservationStatus(r) {
  ...
  if (r.check_in_date && r.check_out_date) return "completed";
  if (r.check_in_date && !r.check_out_date) return "checked_in";
  const now = new Date();
...
function useGingrData(locationId) {
  ...
  const buildRooms = useCallback((resTypes, reservations, roomCounts) => {
    const roomMap = {};
    ...
    const extractRoomTypeName = (name) => {
      if (!name) return null;
      // Remove "Boarding | " prefix and " (All Inclusive)" suffix
      let clean = name.replace(/^Boarding\s*\|\s*/i, "").replace(/\s*\(All Inclusive\)\s*$/i, "").trim();
      return clean || null;
    };

    // Default room counts per type (used when no config is saved)
    ...
    if (boardingTypes.length > 0) {
      ...
      // Fallback to hardcoded ROOM_TYPES if no synced data yet
      ROOM_TYPES.forEach(rt => {
...
function DailyOpsPage({ data, save, sub, nav, profile, addGlobalToast }) {
  const td = todayStr();
  const [viewDate, setViewDate] = useState(td);
  const dayIdx = new Date(viewDate + "T12:00:00").getDay();
...
function GingrIntegrationTab() {
  ...
  return (
    <div>
      ...
      <RoomCountConfig locationId={profile?.location_id} />
    </div>
  );
}

// ─── Room Count Configuration ─────────────────────────────────────────────
function RoomCountConfig({ locationId }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setRoomTypes(types.length > 0 ? types : Object.keys(defaultCounts));
      // Fetch saved room counts
      const { data: setting } = await supabase
        .from("lite_settings").select("setting_value")
        .eq("location_id", locationId).eq("setting_key", "room_counts").maybeSingle();
      setRoomCounts(setting?.setting_value || {});
      setLoading(false);
    };
    load();
  }, [locationId]);
  ...
  const handleSave = async () => {
    ...
    await supabase.from("lite_settings").upsert({
      location_id: locationId,
      setting_key: "room_counts",
      setting_value: roomCounts,
  ...
  return (
    <div style={{ marginTop: 32, paddingTop: 24, borderTop: `1.5px solid ${C.borderLight}` }}>
      <h4 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 700, color: C.text }}>Room Configuration</h4>
      <p style={{ margin: "0 0 16px", fontSize: 13, color: C.textSec, lineHeight: 1.6 }}>
        Set the number of rooms for each boarding type. Room types are pulled from your Gingr reservation types.
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginBottom: 16 }}>
        {roomTypes.map(rt => (
          <div key={rt} style={{ padding: "14px 16px", background: C.bg, borderRadius: 10, border: `1px solid ${C.borderLight}`, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{rt}</span>
            <input
              type="number" min="0" max="50"
              value={roomCounts?.[rt] ?? defaultCounts[rt] ?? 6}
              onChange={e => setRoomCounts(prev => ({ ...prev, [rt]: parseInt(e.target.value) || 0 }))}
              style={{ width: 60, padding: "6px 8px", borderRadius: 6, border: `1.5px solid ${C.border}`, fontSize: 14, fontWeight: 600, textAlign: "center", background: C.surface, color: C.text, fontFamily: "inherit" }}
            />
          </div>
        ))}
      </div>
      <button onClick={handleSave} disabled={saving} style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: saved ? C.suc : C.pri, color: "#fff", fontSize: 13, fontWeight: 600, cursor: saving ? "default" : "pointer", fontFamily: "inherit", transition: "all 0.15s", display: "flex", alignItems: "center", gap: 6 }}>
        {saving ? "Saving..." : saved ? "Saved!" : "Save Room Counts"}
      </button>
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 140px auto", gap: 12, alignItems: "flex-end" }}>
                    ...
                    <div>
                      ...
                      <input type="text" value={inviteName} onChange={e => setInviteName(e.target.value)} placeholder="Jane Vance" style={{ width: "100%", padding: "10px 12px", border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontFamily: "inherit", boxSizing: "border-box" }} />
                    </div>
                    <div>
...
function EnterpriseOpsMatrix() {
  const locations = [
    { id: "ch", name: "Adair Forsythe", opening: 92, frontend: 88, backend: 95, closing: 85, rooms: 100, pictures: 70, privatePlay: 78 },
