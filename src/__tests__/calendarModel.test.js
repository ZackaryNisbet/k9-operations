...
describe("source normalizers", () => {
  ...
  it("labor: emits start + distinct first-shift inside the window only", () => {
    const employees = [
      { id: "e1", full_name: "Alex Smith", position_title: "CSR", start_date: "2026-05-12", first_shift_date: "2026-05-14" },
  ...
  it("reviews: only 30/60/90 cycles, names from the employee map, overdue/done tone", () => {
    const empMap = new Map([["e1", { full_name: "Alex Smith" }]]);
    const instances = [
    ...
    expect(out[0].title).toBe("Alex Smith · 30-Day review");
