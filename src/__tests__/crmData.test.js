...
const bookingLead = {
  ...
  first_name: "Janelle",
  last_name: "Bennett",
  email: "JMBBennett.jmm@gmail.com",
  phone: "8567018139",
...
describe("isAppointment — Gingr appointments excluded from the CRM", () => {
  const appt = { id: "a1", lead_type: "web_form", raw_email_subject: "New Appointment Received | Adair Forsythe", form_data: { estimated_subtotal: "435.00", estimated_tax: "22.26" } };
  ...
  const webform = { id: "w1", lead_type: "web_form", raw_email_subject: "New Web Form Received | Adair Forsythe", form_data: { desired_service: "Daycare" } };
...
describe("general contact-us web forms excluded (not the booking form)", () => {
  const contactLead = { id: "c1", lead_type: "web_form", raw_email_subject: "New Web Form Received | Adair Forsythe", form_data: { first_name: "Ashlynn", last_name: "Masino", form_name: "Contact Form", lead_page_url: "https://www.k9resorts.com/cherry-hill/contact-us/" } };
...
describe("lead change history (global timeline)", () => {
  ...
  const updatesByLead = {
    l1: [
      { id: "u2", lead_id: "l1", update_type: "call", notes: "called", next_follow_up_date: "2026-06-01", created_at: "2026-05-25T10:00:00Z", created_by_name: "Skyler" },
  ...
  it("builds one row per update, newest first, with follow-up transitions", () => {
    ...
    expect(rows[0]).toMatchObject({ leadName: "Pam S", actor: "Skyler", type: "call", prevFollowUp: "2026-05-22", newFollowUp: "2026-06-01" });
  ...
  it("filter options, day metrics, and applied filters (training-parity)", () => {
    ...
    expect(opts.actors.map((o) => o.value)).toEqual(expect.arrayContaining(["Skyler", "Pat", "Ignite"]));
...
describe("canonical form fields — one defined list per category", () => {
  ...
  it("booking renders one fixed, ordered list, normalizing synonym keys", () => {
    ...
    expect(fields.find((f) => f.key === "email").value).toBe("JMBBennett.jmm@gmail.com");
