...
<table cellpadding="0" cellspacing="0"><tbody>
  ...
  <tr><th>Last Name: </th><td><div>Bennett</div></td></tr>
  <tr><th>Email: </th><td><div>JMBBennett.jmm@gmail.com</div></td></tr>
...
describe("parseRegex on the booking form (th/td rows)", () => {
  ...
  it("extracts the labelled fields", () => {
    ...
    expect(fields.last_name).toBe("Bennett");
    expect(fields.email).toBe("JMBBennett.jmm@gmail.com");
...
describe("parseIgniteEmail on the booking form", () => {
  ...
  it("still parses when forwarding has rewritten the sender", () => {
    const forwarded = parseIgniteEmail(
      BOOKING_FORM_HTML,
      { from: "Skyler <zack@gmail.com>", subject: "Fwd: New Booking Form Submission Received" },
      { useRegex: true }
    );
    expect(forwarded.error).toBeUndefined();
    expect(forwarded.clientName).toBe("Janelle Bennett");
