...
describe("splitPersonName", () => {
  it("splits a multi-token name into given name + surname", () => {
    expect(splitPersonName("Dr. Jane A. Smith")).toEqual({ first_name: "Dr. Jane A.", last_name: "Smith" });
  });
...
describe("display helpers", () => {
  ...
  it("composes and falls back contact names", () => {
    expect(getDirectoryContactName({ first_name: "Jane", last_name: "Smith" })).toBe("Jane Smith");
    expect(getDirectoryContactName({})).toBe("Unnamed contact");
  });
  it("derives initials", () => {
    expect(getDirectoryContactInitials({ first_name: "Jane", last_name: "Smith" })).toBe("JS");
    expect(getDirectoryContactInitials({})).toBe("?");
  });
  it("prefers the split address, falling back to the raw address", () => {
    expect(getDirectoryOrgAddressText({ address_line_1: "1 Main St", address_city: "Deerfield", address_state: "IL" }))
      .toBe("1 Main St · Deerfield, IL");
...
describe("directory entry assembly", () => {
  ...
  const contacts = [
    { id: CON_A, org_id: ORG_A, first_name: "Jane", last_name: "Smith", title: "Manager" },
