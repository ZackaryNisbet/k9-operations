...
describe("applyLaborRosterFilters", () => {
  ...
  it("copies roster values through clipboard API and textarea fallback", async () => {
    ...
    await expect(copyTextToClipboard("  Jane Vance <jane@example.com>  ", {
      navigator: { clipboard: { writeText } },
    })).resolves.toBe(true);
    expect(writeText).toHaveBeenCalledWith("Jane Vance <jane@example.com>");
  ...
  it("preserves the current labor model when creating the default active saved model", () => {
    ...
    expect(payload).toMatchObject({
      ...
      name: "Current Adair Forsythe Operating Model",
      is_active: true,
  ...
  it("filters Training History by date, employee, category/task, and actor with day metrics", () => {
    ...
    expect(options.employees.map((option) => option.label)).toEqual(["All employees", "Amanda Drill", "Casey Gonzalez"]);
    expect(options.actors.map((option) => option.label)).toEqual(["All actors", "Angelina DeAugustine", "Skyler Brooks"]);
      ...
      events: [
        {
          ...
          training_records: {
            id: "record-1",
            labor_employee_id: "employee-1",
            employee_full_name: "Casey Gonzalez",
            template_name_snapshot: "PCT Team Readiness Board",
          },
