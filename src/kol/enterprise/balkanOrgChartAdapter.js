...
const EXCLUDED_DUPLICATE_EXECUTIVES = new Set(["group:co-ceos", "person:logan-hale-1", "person:finley-prescott-21"]);
...
function shortenTitle(title = "", location = "") {
  ...
  if (/Director of Resorts/i.test(title) && /Adair Forsythe/i.test(location)) return "Director of Resorts | GM Adair Forsythe";
...
function nodeTags(node) {
  ...
  if (node.id === "person:cameron-rhodes-15") tags.add("k9-coo");
  if (node.id === "person:rowan-beckett-25") tags.add("k9-resort-ops");
  if (node.id === "person:aubrey-nolan-28") tags.add("k9-director");
  return [...tags];
}
