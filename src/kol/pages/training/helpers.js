...
import { fmtDateFull, todayStr } from "../../../shared/theme";
import { getCompletionEvidence } from "../../components/PerformanceReviewComplianceGrid";
import { LABOR_EMPLOYEE_ATTACHMENT_BUCKET, getLaborEmploymentCommitmentLabel } from "../../trainingData";
import { TRAINING_GRACE_PERIOD_DAYS } from "./constants";
...
export function getLaborModelCoverageRoleOptionForCell(value = "", rowGroupKey = "") {
  const normalized = normalizeLaborModelCoverageCell(value);
  if (!normalized || normalized === LABOR_MODEL_MARKETING_COVERAGE_VALUE) return null;
...
export function normalizeHourAnalysisLaborModelDay(dayKey, value = {}, fallback = {}) {
  const source = isObjectRow(value) ? value : {};
...
export function calculateLaborModelRowHourBuckets(row = {}, columns = []) {
...
export function buildComplianceRequirementSlug(value = "", existingRequirements = []) {
...
export function getReviewCycleEvidenceDocument(reviewCycle = {}, documentsById = {}) {
...
export function getRestartedReviewStatus(dueDate) {
  if (dueDate && String(dueDate).slice(0, 10) < todayStr()) return "overdue";
  return "scheduled";
}

export function getLaborModelRolePalette(groupKey = "", roleColors = null) {
  const normalizedGroup = normalizeLaborModelGroupKey(groupKey);
...
export function calculateLaborModelRowHours(row = {}, columns = []) {
...
export function getDaysSince(dateValue) {
...
export function getLaborModelCoverageRoleStyle(groupKey = "", roleColors = null) {
...
export function getLaborModelNextCoverageValue(value = "", rowGroupKey = "") {
  const kind = getLaborModelCoverageKind(value);
...
export function getTrainingComplianceState(row) {
...
export function buildComplianceHistoryDetailParts({ event = {}, reviewInstance = {}, checkpointLabel = "" } = {}) {
  const previousState = readComplianceHistoryState(event, "old");
  const nextState = readComplianceHistoryState(event, "new");
...
export function buildComplianceAuditDetailParts(event = {}) {
  const before = readComplianceAuditSnapshot(event, "before_snapshot");
  const after = readComplianceAuditSnapshot(event, "after_snapshot");
