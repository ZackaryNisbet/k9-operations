      ...
      <Card style={{ padding: 16, marginBottom: 16, border: `1.5px solid ${C.pri}` }}>
        ...
        {editBody && (
          <div style={{ marginBottom: 12, padding: "10px 12px", borderRadius: 8, background: C.bg, border: `1px solid ${C.borderLight}` }}>
            ...
            <div style={{ fontSize: 12, color: C.text }}>{editBody.replace(/\{clientName\}/g, "Jane Smith").replace(/\{dogName\}/g, "Buddy and Max").replace(/\{checkInDate\}/g, "Mar 15").replace(/\{checkOutDate\}/g, "Mar 20").replace(/\{roomType\}/g, "Executive Suite").replace(/\{totalPrice\}/g, "450.00")}</div>
          </div>
        )}
