...
function computeLevel(s: {
  bridgeOk: boolean; resendOk: boolean | null; dbOk: boolean; roundtripOk: boolean | null; lastLeadAt: string | null; now: Date;
  ...
  if (s.lastLeadAt) {
    const days = Math.floor((s.now.getTime() - new Date(s.lastLeadAt).getTime()) / 86400000);
    if (days > STALE_DAYS) return { level: "warn", detail: `No booking forms in ${days} days — check the Outlook forwarding rule.` };
  }
  if (s.roundtripOk === true) return { level: "ok", detail: "Pipeline verified end-to-end — synthetic round-trip landed." };
