-- ============================================================================
-- Fix: Invite race condition — replace single pg_sleep(1) with retry loop
-- ============================================================================
-- The previous send_lite_invite used net.http_post (async) to create the auth
-- user, then slept 1 second and checked auth.users once. Under load the user
-- often isn't ready in time, producing:
--   "Auth user creation may still be processing. Try again in a moment."
--
-- This migration replaces the function with one that:
--   1. Polls auth.users up to 10 times (0.5 s apart, ~5 s total max wait)
--   2. Checks the pg_net response status to detect HTTP errors early
--   3. Handles "user already exists" 422 responses gracefully
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    WHERE id = v_user_id;
  ELSE
    -- Create new auth user via Supabase Admin API (async via pg_net)
    ...
    -- ── Poll for the auth user with retries ──────────────────────────────
    -- net.http_post is async; the user may not appear in auth.users
    -- immediately. We poll up to 10 times, 0.5 s apart (~5 s max).
    v_user_id := NULL;
    FOR poll_attempt IN 1..10 LOOP
      PERFORM pg_sleep(0.5);

      SELECT id INTO v_user_id FROM auth.users WHERE email = invite_email;
      EXIT WHEN v_user_id IS NOT NULL;

      -- After a few attempts, check if the HTTP request itself failed
      IF poll_attempt = 4 THEN
        BEGIN
          SELECT status_code, (response).body::TEXT
          INTO http_status, http_response_body
          FROM net._http_response
          WHERE id = request_id;

          -- If we got a response and it's an error, bail early
          IF http_status IS NOT NULL AND http_status >= 400 THEN
            -- 422 often means "user already exists" — re-check auth.users
            IF http_status = 422 THEN
              SELECT id INTO v_user_id FROM auth.users WHERE email = invite_email;
              IF v_user_id IS NOT NULL THEN
                -- User was created by a concurrent request; update password
                UPDATE auth.users SET
                  encrypted_password = crypt(temp_pass, gen_salt('bf')),
                  raw_user_meta_data = COALESCE(raw_user_meta_data, '{}'::jsonb) || jsonb_build_object(
                    'full_name', COALESCE(NULLIF(invite_name, ''), raw_user_meta_data->>'full_name')
                  ...
                  updated_at = now()
                WHERE id = v_user_id;
                EXIT; -- break out of loop, we have the user
              END IF;
            END IF;

            RETURN jsonb_build_object(
              'success', false,
              'error', 'Failed to create auth user (HTTP ' || http_status || '). Please try again.'
            );
          END IF;
        EXCEPTION WHEN OTHERS THEN
          -- pg_net response table may not be populated yet; continue polling
          NULL;
        END;
      END IF;
    END LOOP;
    ...
    IF v_user_id IS NULL THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Auth user creation timed out after 5 seconds. Please try again.'
      );
    END IF;
  END IF;
