...
workflow_meta AS (
  SELECT * FROM (
    VALUES
      ...
      ('roll-call-opening', 'Opening Roll Call', 'supervisor', 9),
      ('roll-call-closing', 'Closing Roll Call', 'supervisor', 10)
...
roll_call_opening_progress AS (
  SELECT
    ...
    'Opening Roll Call'::TEXT AS title,
    ...
    oro.computed_at
  FROM ops_roll_call_opening oro
),
roll_call_closing_progress AS (
  SELECT
    'roll-call-closing'::TEXT AS id,
    'Closing Roll Call'::TEXT AS title,
    ...
    orc.computed_at
  FROM ops_roll_call_closing orc
),
