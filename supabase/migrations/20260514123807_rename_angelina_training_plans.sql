-- Rename the readiness templates from "Training Guide" to "Training Plan"
-- while preserving the stable slugs that clients and RPCs use.
WITH renamed_templates AS (
  UPDATE public.training_templates template
  SET
    name = CASE template.slug
      WHEN 'pct_team_readiness_board' THEN 'Angelina''s PCT Training Plan v1'
      WHEN 'csr_team_readiness_board' THEN 'Angelina''s CSR Training Plan v1'
      ELSE template.name
    END,
    updated_at = now()
...
renamed_records AS (
  ...
  SET
    template_name_snapshot = renamed_templates.name,
    template_snapshot = jsonb_set(
      COALESCE(record.template_snapshot, '{}'::jsonb),
      '{template,name}',
...
SET published_snapshot = jsonb_set(
  COALESCE(version.published_snapshot, '{}'::jsonb),
  '{template,name}',
  to_jsonb(renamed_templates.name),
  true
)
