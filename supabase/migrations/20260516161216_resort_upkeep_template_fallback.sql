-- Keep checklist period creation resilient when a location-specific template row
-- exists but has no published active version yet. The global template must remain
-- a valid fallback so one bad override cannot break an entire frequency.
...
WITH latest_published AS (
  ...
  ORDER BY template_id, version_number DESC, published_at DESC NULLS LAST
)
UPDATE public.resort_upkeep_templates t
SET active_version_id = latest_published.version_id
FROM latest_published
WHERE latest_published.template_id = t.id
  AND (
...
BEGIN
  ...
  IF v_template.active_version_id IS DISTINCT FROM v_version.id THEN
    UPDATE public.resort_upkeep_templates
    SET active_version_id = v_version.id
    WHERE id = v_template.id;
    v_template.active_version_id := v_version.id;
  END IF;
