-- Require local evidence when completing policy-backed performance review checkpoints.

BEGIN;

UPDATE public.labor_compliance_requirements
SET
  evidence_policy = 'file_required',
  updated_at = now(),
  updated_by_user_id = COALESCE(updated_by_user_id, created_by_user_id)
WHERE requirement_kind = 'review_checkpoint'
  AND evidence_policy = 'checkbox_only'
  AND (
    metadata ? 'legacy_review_cycle'
    OR slug ~ '^review_[0-9]+_day$'
  );

DROP FUNCTION IF EXISTS public.complete_employee_review_instance(uuid, uuid, text);
...
BEGIN
  ...
  SET
    status = 'completed',
    completed_at = COALESCE(completed_at, v_completed_on::timestamptz, now()),
...
COMMENT ON FUNCTION public.complete_employee_review_instance(uuid, uuid, text, uuid, date) IS
  'Completes a performance review only after a local labor_employee_documents evidence file is linked to the configured compliance review checkpoint.';
