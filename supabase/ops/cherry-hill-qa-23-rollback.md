# Adair Forsythe QA 23 Rollback Notes
...
```bash
npm run supabase:functions:rollback -- --project-ref YOUR_SUPABASE_PROJECT_REF --functions compute-scheduling-matrix,gingr-sync --commit <previous-good-sha> --note "Rollback Adair Forsythe QA 23 scheduling changes"
```
...
- Re-apply the helper definitions from
  `20260511164000_labor_interview_identity_privacy.sql`.
- Verify interview identity access for UUID-backed Adair Forsythe profiles before
